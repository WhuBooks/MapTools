...
#ifndef MAPTOOLS_CLOTHOID_HPP
#define MAPTOOLS_CLOTHOID_HPP
...
namespace maplib
{
    namespace curve{
        ...
        const std::vector<double> fds = {
                1.0, 2.7022305772400260215, 4.2059268151438492767, 4.5221882840107715516,
                3.7240352281630359588, 2.4589286254678152943, 1.3125491629443702962,
                0.5997685720120932908, 0.20907680750378849485, 0.07159621634657901433,
                0.012602969513793714191, 0.0038302423512931250065};
    
        const std::vector<double> gns = {
        ...
        const std::vector<double> gds = {
                1.0, 2.0646987497019598937, 2.9109311766948031235, 2.6561936751333032911,
                2.0195563983177268073, 1.1167891129189363902, 0.57267874755973172715,
                0.19408481169593070798, 0.07634808341431248904, 0.011573247407207865977,
                0.0044099273693067311209, -0.00009070958410429993314};

        
        const double eps1=2.2204e-016;
        const int inf=10000000;
        const int MAXD=100000000;
        const int MINDIS=4;
        ...
        template<typename T>
        std::vector<T> clothoid(const T &s, const T &e, int npts, double tol)
        {
            double sx=s.m_x,sy=s.m_y,syaw=s.m_yaw;
            double ex=e.m_x,ey=e.m_y,eyaw=e.m_yaw;
            ...
            double phi0 =util::degree2Grad(syaw) - phi;
            ...
            double phi1 =util::degree2Grad(eyaw) - phi;
            while(phi1>M_PI)
                phi1=phi1-2*M_PI;
            while(phi1<=-M_PI)
                phi1=phi1+2*M_PI;
            ...
            double s_e_dis=std::sqrt(dx*dx+dy*dy);
            if (std::abs(Kappa) > 0.5 && std::abs(Kappa + DKappa * L) > 0.5 && L > 1.5 * s_e_dis && L<10*s_e_dis)
                return std::vector<T>();
            ...
            for (double t = 0; t <= L;)
            {
                ...
                intXY(C, S, 1, DKappa * t * t, Kappa * t, util::degree2Grad(syaw));

                double tmp_x = sx + t * C[0];
                double tmp_y = sy + t * S[0];
                double tmp_yaw = syaw + util::grad2Degree(t * Kappa + 0.5 * t * t * DKappa);
                XY.push_back(T(tmp_x, tmp_y, tmp_yaw));
