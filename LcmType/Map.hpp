...
#ifndef __nox_lcm_Map_hpp__
#define __nox_lcm_Map_hpp__
...
class Map
{
    public:
        ...
        std::vector< nox_lcm::Block > blocks;
...
int Map::_encodeNoHash(void *buf, int offset, int maxlen) const
{
    ...
    tlen = __int32_t_encode_array(buf, offset + pos, maxlen - pos, &this->block_num, 1);
    if(tlen < 0) return tlen; else pos += tlen;

    for (int a0 = 0; a0 < this->block_num; a0++) {
        tlen = this->blocks[a0]._encodeNoHash(buf, offset + pos, maxlen - pos);
        if(tlen < 0) return tlen; else pos += tlen;
    }
...
int Map::_decodeNoHash(const void *buf, int offset, int maxlen)
{
    ...
    tlen = __int32_t_decode_array(buf, offset + pos, maxlen - pos, &this->block_num, 1);
    if(tlen < 0) return tlen; else pos += tlen;

    this->blocks.resize(this->block_num);
    for (int a0 = 0; a0 < this->block_num; a0++) {
        tlen = this->blocks[a0]._decodeNoHash(buf, offset + pos, maxlen - pos);
        if(tlen < 0) return tlen; else pos += tlen;
    }
...
int Map::_getEncodedSizeNoHash() const
{
    ...
    for (int a0 = 0; a0 < this->block_num; a0++) {
        enc_size += this->blocks[a0]._getEncodedSizeNoHash();
    }
    enc_size += __int32_t_encoded_array_size(NULL, 1);
    return enc_size;
}
...
uint64_t Map::_computeHash(const __lcm_hash_ptr *p)
{
    ...
    for(fp = p; fp != NULL; fp = fp->parent)
        if(fp->v == Map::getHash)
            return 0;
    const __lcm_hash_ptr cp = { p, (void*)Map::getHash };

    uint64_t hash = 0x873674c94d10dcf8LL +
         nox_lcm::Block::_computeHash(&cp);
